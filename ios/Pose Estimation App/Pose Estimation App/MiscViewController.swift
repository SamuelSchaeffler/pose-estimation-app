...
class MiscViewController: UIViewController {
    ...
    var filterVC = LandmarkFilterViewController()
    ...
    var filterButton: UIButton = {
        let button = UIButton()
        let symbolConfiguration = UIImage.SymbolConfiguration(pointSize: 20, weight: .bold)
        let image = UIImage(systemName: "lines.measurement.horizontal", withConfiguration: symbolConfiguration)?.withTintColor(.white, renderingMode: .alwaysOriginal)
        button.setImage(image, for: .normal)
        button.adjustsImageWhenHighlighted = false
        button.setTitle("Landmarken-Filter", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 20, weight: .semibold)
        ...
        let buttonWidth: CGFloat = UIScreen.main.bounds.size.width - 120
        let buttonHeight: CGFloat = 60
        button.frame = CGRect(x: (UIScreen.main.bounds.size.width - buttonWidth) / 2, y: UIScreen.main.bounds.size.height - 300, width: buttonWidth, height: buttonHeight)
        button.layer.cornerRadius = 30
        button.addTarget(self, action: #selector(buttonPressed), for: .touchDown)
        button.addTarget(self, action: #selector(buttonReleased), for: .touchUpInside)
        button.addTarget(self, action: #selector(buttonReleased), for: .touchUpOutside)
    ...
    @objc func openFilter() {
        self.filterVC.modalPresentationStyle = .fullScreen
